...
namespace MGS3_MC_Cheat_Trainer
{
    ...
    : base(name, IntPtr.Zero) // Initially, we don't have the address
        ...
        public void LogAllWeaponsAndItemsAddresses()
        {
            ...
            foreach (var weapon in weapons)
            {
                ...
                if (weapon.SuppressorToggleOffset != IntPtr.Zero)
                {
                    ...
                    LoggingManager.Instance.Log($"Suppressor Toggle Address: {suppressorToggleAddress.ToString("X")}");
                }
            }
            ...
            foreach (var item in items)
            {
                ...
                LoggingManager.Instance.Log($"{item.Name} - Address: {itemAddress.ToString("X")}");

                if (item.MaxCapacityOffset != IntPtr.Zero)
                {
                    IntPtr maxCapacityAddress = ItemAddresses.GetMaxAddress(itemAddress);
                    LoggingManager.Instance.Log($"Max Capacity Address: {maxCapacityAddress.ToString("X")}");
                }
            }
